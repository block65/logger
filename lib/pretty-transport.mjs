import { bgRed, bgYellow, black, blue, bold, gray, green, red, whiteBright, } from 'colorette';
...
export function createPrettifier( /*options?: unknown*/) {
    return (log) => {
        ...
        const formattedRest = Object.keys(rest).length > 0
            ? ' ' +
            util.inspect(rest, {
                colors: 'FORCE_COLOR' in process.env ||
                    (isatty(1) &&
            ...
            : '';
        return `${gray(new Date(time).toJSON())} ${formatLevel(level).padEnd(6, ' ')}${formattedName} ${formattedMsg}${formattedRest}\n`;
    };
}
export default async function (options) {
    ...
    const destination = new SonicBoom({
        fd: typeof options?.destination === 'number'
            ? options?.destination
            : undefined,
        dest: typeof options?.destination !== 'number'
